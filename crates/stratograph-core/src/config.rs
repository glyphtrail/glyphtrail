...
/// Per-repo layout. The index lives in `<repo>/.stratograph/`.
pub const INDEX_DIR: &str = ".stratograph";
pub const DB_FILE: &str = "graph.db";
pub const IGNORE_FILE: &str = ".stratographignore";
/// Optional per-repo config file, read from `<repo>/.stratograph/config.toml`.
...
/// Per-repo configuration, read from `.stratograph/config.toml`. Every field has a
