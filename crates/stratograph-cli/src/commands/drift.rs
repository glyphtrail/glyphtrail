//! `stratograph drift` — reconcile code-first API endpoints against the blessed
...
use stratograph_core::{EdgeKind, NodeKind, OperationKey};
    ...
    if !report.schema_present {
        println!(
            "No blessed schema configured ([[api.schemas]]); nothing to reconcile \
             (see `stratograph query endpoints` for the indexed routes)."
        );
        return;
    }
...
mod tests {
    use super::*;
    use assert2::check;
    use stratograph_core::{Confidence, Edge, HttpMethod, Node, NodeId, Span};
