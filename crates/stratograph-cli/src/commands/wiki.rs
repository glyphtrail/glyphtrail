//! `stratograph wiki` — generate a docs wiki from the knowledge graph via an LLM
...
pub fn run(args: WikiArgs) -> Result<()> {
    ...
    if args.target == Target::Serve && args.dry_run {
        ...
        return rt.block_on(stratograph_server::serve_wiki(
            args.output.clone(),
            args.port,
        ));
    }
    ...
    if args.target == Target::Serve {
        let rt = tokio::runtime::Runtime::new()?;
        rt.block_on(stratograph_server::serve_wiki(
            args.output.clone(),
            args.port,
        ))?;
    }
...
fn pages_config(title: &str) -> String {
    format!(
        "title: {title}\ndescription: Documentation generated by stratograph from the code graph.\ntheme: jekyll-theme-cayman\n"
    )
}
...
fn operations_facts(
    ...
    for (id, key) in ops {
        ...
        let handler = if with_handler {
            store
                .neighbors(&id.0, Some(stratograph_core::EdgeKind::Handles), false)?
                .into_iter()
                .next()
...
mod tests {
    ...
    fn pages_compose_from_graph_facts() {
        use stratograph_core::{Edge, Node, NodeId, Span};
