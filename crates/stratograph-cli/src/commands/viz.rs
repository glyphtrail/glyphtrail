...
use stratograph_core::{ImpactPolicy, ImpactReport, Node, NodeId, OperationKey};
use stratograph_store::GraphStore;
use stratograph_viz::ImpactView;
...
pub fn run(
    ...
    let html = stratograph_viz::static_html(&nodes, &edges, &ops);
...
fn run_impact(
    ...
    let html = stratograph_viz::static_html_impact(&nodes, &edges, &ops, &view);
    std::fs::write(output, html)?;
    println!(
