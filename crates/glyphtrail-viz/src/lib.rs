...
use glyphtrail_core::{Edge, ImpactReport, Node, NodeId, OperationKey};
...
pub fn to_elements(
    ...
    let seed_set: std::collections::HashSet<&str> = impact
        ...
        .unwrap_or_default();
    let item_by_id: HashMap<&str, &glyphtrail_core::ClassifiedItem> = impact
...
fn render(data: Value, impact: Option<Value>) -> String {
    let mut inject = format!(
        "<script>window.GLYPHTRAIL_DATA = {};",
        serde_json::to_string(&data).unwrap_or_else(|_| "[]".into())
    );
    if let Some(s) = impact {
        inject.push_str(&format!(
            "window.GLYPHTRAIL_IMPACT = {};",
