//! `glyphtrail story` — narrate a repository's history from its git log via an
...
use clap::Args;
use glyphtrail_core::{Registry, RegistryEntry, default_registry_path};
...
mod tests {
    ...
    fn story_prompt_summarizes_span_and_contributors() {
        ...
        let p = story_prompt("glyphtrail", &commits);
        check!(p.contains("Repository: glyphtrail"));
