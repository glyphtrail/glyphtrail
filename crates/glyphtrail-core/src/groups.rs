...
//! `~/.glyphtrail/groups.json`. A group scopes a federated operation
...
//! the per-repo `.glyphtrail/` index stays the source of truth. The file is
...
/// The default groups path, `~/.glyphtrail/groups.json`, from `HOME` (or
...
pub fn default_groups_path() -> Option<PathBuf> {
    ...
    Some(home.join(".glyphtrail").join("groups.json"))
}
...
mod tests {
    ...
    fn save_then_load_roundtrips() {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("glyphtrail-groups-{nanos}/groups.json"));
    ...
    fn mutate_serializes_concurrent_writers() {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("glyphtrail-groups-lock-{nanos}"));
