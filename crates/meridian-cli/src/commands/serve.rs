...
pub fn run(repo: &Path, port: u16) -> Result<()> {
    ...
    // `.meridian/ladybug` index beside this anchor path (#165). Passing the
    // canonical `graph.db` path lets it locate the `.meridian` dir.
    let rt = tokio::runtime::Runtime::new()?;
    rt.block_on(meridian_server::serve(store, paths.db_path.clone(), port))
}
