...
impl LadybugStore {
    ...
    pub fn open_temp() -> Result<Self> {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        let dir =
            std::env::temp_dir().join(format!("stratograph-lbug-{}-{nanos}", std::process::id()));
        Self::open(&dir)
    }
...
fn parse_proto(s: &str) -> stratograph_core::Protocol {
    use stratograph_core::Protocol;
    match s {
        "grpc" => Protocol::Grpc,
...
fn op_from_row(row: &[Value]) -> (NodeId, OperationKey) {
    let method = opt(get_str(row, 2)).and_then(|m| stratograph_core::HttpMethod::parse(&m));
    (
...
mod tests {
    ...
    fn fixture_roundtrip_and_traversal() {
        ...
        lb.insert_operations(&[(
            NodeId("a".into()),
            stratograph_core::OperationKey::rest(stratograph_core::HttpMethod::Get, "/x"),
        )])
        .unwrap();
        lb.insert_imports(&[("a.rs".into(), "b".into(), "rust".into())])
            .unwrap();
        lb.insert_pending(&[stratograph_core::PendingLink {
